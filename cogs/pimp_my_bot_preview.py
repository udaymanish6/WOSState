...
class ThemePreviewView(discord.ui.View):
    ...
    def build_settings_menu_preview(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"❄️ Whiteout Survival — Control Panel",
            description=(
                f"{castle_icon} **Alliance Operations** — manage alliances\n"
                f"{members_icon} **Member Operations** — add, remove, view members\n"
                f"{robot_icon} **Bot Operations** — admins & configuration\n"
                f"{gift_icon} **Gift Codes** — redeem & manage codes\n"
                f"{list_icon} **History** — furnace & nickname logs\n"
                f"{settings_icon} **Other Features** — extra tools"
            ),
            color=color
        )
        embed.set_footer(text=f"❄️ Whiteout Survival Bot • Preview: {self.PAGE_TITLES[self.current_page]} • Theme: {self.session.theme_name}")
        return embed
